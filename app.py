...
# --- CSS Personnalisé (Dark Mode Premium & Cartes Netflix) ---
st.markdown("""
<style>
    .stApp {
...
objectif = st.sidebar.selectbox("Objectif nutritionnel", ["Prise de masse (Protéiné)", "Sèche / Maintien (Équilibré)", "Économique & Rapide"])
nb_repas = st.sidebar.slider("Nombre de repas à planifier", 1, 30, 7)
budget_max = st.sidebar.number_input("Budget total max (€)", min_value=5.0, max_value=500.0, value=50.0, step=5.0)

st.sidebar.markdown("---")
st.sidebar.subheader("🧊 Anti-Gaspillage / Vide-Frigo")
vide_frigo = st.sidebar.text_input("Ingrédients à utiliser en priorité", placeholder="Ex: Œufs, Riz, Tomates...")
...
def generer_menu_ia(liste_produits, objectif, count, budget, frigo=""):
    ingredients_dispo = [f"{p.get('item', '')} ({p.get('prix', '')}€)" for p in liste_produits[:50]] if liste_produits else ["Poulet", "Riz", "Légumes", "Œufs"]
...
st.caption("Planification de repas sur-mesure, anti-gaspillage, planning hebdomadaire et liens Carrefour.")
...
if not api_key:
    ...
    if st.button("✨ Générer mon menu personnalisé", type="primary", use_container_width=True):
        with st.spinner("L'IA prépare vos recettes et calcule les dépenses..."):
            try:
                st.session_state["menu"] = generer_menu_ia(produits, objectif, nb_repas, budget_max, vide_frigo)
...
if "menu" in st.session_state and st.session_state["menu"]:
    ...
    col_a.metric("Total estimé", f"{cout_total:.2f} €", delta=f"{budget_max - cout_total:.2f} € de marge")
    ...
    tab_menu, tab_planning, tab_courses = st.tabs(["🍽️ Catalogue de Repas", "📅 Planning Semaine", "🛒 Liste de Courses Globale"])

    # --- TAB 1 : CATALOGUE & REPLACEMENT ---
    with tab_menu:
        cols = st.columns(3)
        for idx, r in enumerate(recettes):
            with cols[idx % 3]:
                ...
                st.markdown(f"<span class='badge-price'>💰 ~{r.get('prix_estime', 0):.2f} €</span> <span class='badge-time'>⏱️ {r.get('temps')}</span>", unsafe_allow_html=True)
                ...
                # Option : Remplacer uniquement cette recette
                if st.button(f"🔄 Changer ce plat", key=f"swap_{idx}"):
                    with st.spinner("Recherche d'une alternative..."):
                        nouvelle_recette = remplacer_une_recette(r, objectif, budget_max/len(recettes))
                ...
                with st.expander("📖 Voir la recette complète & liens Carrefour"):
                    ...
                    for ing in r.get("ingredients", []):
                        ...
                        query = ing.get("recherche_carrefour", nom_ing) if isinstance(ing, dict) else nom_ing
                    ...
                    c1.metric("Calories", f"{macros.get('calories',0)}")
                    c2.metric("Protéines", f"{macros.get('proteines',0)}g")
                    c3.metric("Glucides", f"{macros.get('glucides',0)}g")
                    c4.metric("Lipides", f"{macros.get('lipides',0)}g")
    ...
    with tab_planning:
        ...
        for idx, r in enumerate(recettes):
            ...
            st.markdown(f"**{jour} ({moment})** : {r.get('nom')} — *~{r.get('prix_estime',0):.2f} €*")

    # --- TAB 3 : LISTE DE COURSES CONSOLIDÉE ---
    with tab_courses:
        ...
        texte_export = f"LISTE DE COURSES - BUDGETGOURMET ({len(recettes)} repas)\nTotal estimé : {cout_total:.2f} €\n\n"

        for rayon, items in liste_rayons.items():
            st.markdown(f"#### 📦 {rayon}")
            texte_export += f"--- {rayon} ---\n"
            for item in set(items):  # Filtre les doublons
                st.write(f"- [ ] {item}")
                texte_export += f"- {item}\n"
            texte_export += "\n"
